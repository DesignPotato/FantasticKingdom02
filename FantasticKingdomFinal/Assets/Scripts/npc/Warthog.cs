...
public class Warthog : Unit {
	...
	private EnemyHealth warthogHealth;
    ...
    public override void Start()
    {
		...
		warthogHealth = GetComponent<EnemyHealth> (); 
        //Some initial values
        health = HEALTHSTAT;
        armour = ARMOURSTAT;
        magResist = MAGSTAT;
        ...
        warthogSound = GetComponent<AudioSource>();
    ...
    {	
		if (warthogHealth.currentHealth > 0) {


			//Run away once warthog has gold
			//Debug.Log (warthogHealth.currentHealth);
...
//        
    ...
    {   
        ...
        if (warthogHealth.currentHealth > 0) {
            if (!warthogSound.isPlaying){
         
                selectedNoise = attackNoises[Random.Range(0, attackNoises.Length)];
                warthogSound.PlayOneShot(selectedNoise);
            }
